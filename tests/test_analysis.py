...
from utils.fairness_functions import (cdf, add_decile, agg_psi, PSI, score_percentile_comparison)
...
def test_score_percentile_comparison_returns_valid_structures(mock_dataframe):
    df_quantiles, df_regression = score_percentile_comparison(
        mock_dataframe, protected_variable="Protected", score="Score",
        favoured_class=0, deprived_class=1, plot=False
    )
